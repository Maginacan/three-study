...
function createConvas(text, width) {
...
function createLyricItem(text) {
    const texture = new THREE.CanvasTexture(createConvas(text, text.length * 30));
...
fetch('../../public/superman.lrc').then(res => {
    return res.text();
}).then(content => {
